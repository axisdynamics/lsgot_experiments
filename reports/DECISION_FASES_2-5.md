...
- [x] Identidad del primer token controlada con **token forzado** — **corrección: esto ya estaba corrido, me había equivocado.** `LSGOT_v4/evidence/REPORTE_FASE0.md` §1C ejecutó el forward-pass definitivo (token forzado "The") en `deepseek`, `gemma-it` (Gemma-4-E4B-it) **y `gemma-base` (Gemma-4-E4B)** — exactamente el par usado en Exp. 0.2/0.3 de este documento. Lo que había leído como "sin correr" era una nota de `lsgot_4.md` §3.4 sobre un panel *distinto* (Gemma-4-31B-it), donde sí sigue pendiente — no apliqué bien el alcance. **Hallazgo importante que sí afecta a Exp. 0.2/0.3:** el resultado depende de la convención de v₁. Con v₁ PRE-token (última posición del prompt, `arr[:,0,:]` — la que usa `exp02_baselines_norm_r.py`, confirmado en el código de `hidden_state_extractor.py`), la inversión de norma es causalmente inmune al confound (el token 1 no puede influir hacia atrás) — el control es estructuralmente trivial para esta lectura, no hace falta repetirlo. Con v₁ POST-token (estado del primer token YA generado), el control **revierte** la inversión en `gemma-it` (con token constante, axis 298.1 > vanilla 178.8, d=+9.4) y en `deepseek` — la dirección se invierte completamente respecto de lo pre-token. `REPORTE_FASE0.md` es explícito: *"el paper debe declarar explícitamente la convención y reportar ambas cantidades"* — pendiente todavía para cualquier escrito de la v3, y hay que dejar claro en `EXP02_REPORT.md`/`EXP03_REPORT.md` que ambos usan la convención pre-token, no la única posible
