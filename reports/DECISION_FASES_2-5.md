...
- [x] Identidad del primer token controlada con **token forzado** — cerrado. `LSGOT_v4/evidence/REPORTE_FASE0.md` §1C ejecutó el forward-pass definitivo (token forzado "The") en `deepseek`, `gemma-it` (Gemma-4-E4B-it) y `gemma-base` (Gemma-4-E4B) — exactamente el par usado en Exp. 0.2/0.3. El resultado depende de la convención de v₁: con v₁ PRE-token (`arr[:,0,:]`, la que usa `exp02_baselines_norm_r.py`) el control es causalmente trivial — el token 1 no puede influir hacia atrás, la inversión de norma es inmune al confound por construcción. Con v₁ POST-token (estado del primer token ya generado), el control revierte la inversión en `gemma-it` (axis 298.1 > vanilla 178.8, d=+9.4) y en `deepseek`. Cualquier escrito de la v3 debe declarar explícitamente qué convención usa — `EXP02_REPORT.md`/`EXP03_REPORT.md` ya lo aclaran (pre-token) en su §0
