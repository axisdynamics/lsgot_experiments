"""
MIA Geometric Validation — Gemma 4 31B-it
=========================================
Experimento base MIA (~5K chars) en modelo 31B dense.
...
# ─── Resto del pipeline (idéntico al template) ──────────────────────────────
...
def phase_extract(groups, prompts, results_dir, cache_dir, max_new_tokens, hf_token,
    ...
    print("\n" + "="*60)
    print("FASE 1 — Extracción de Hidden States (MIA)")
    print("="*60)
...
def phase_analyze(groups, prompts, results_dir):
    ...
    results_dir = Path(results_dir)
    print("\n" + "="*60)
    print("FASE 2 — Análisis Geométrico (MIA)")
    print("="*60)
    ...
    if traj_metrics_agg:
        print("\n" + "="*60)
        print("MÉTRICAS DE TRAYECTORIA (MIA)")
        print("="*60)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description=f"MIA on {MODEL_CONFIG['hf_id']}")
